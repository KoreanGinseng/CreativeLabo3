...
namespace Sample {
	...
	public:
		...
		void FromJson(nlohmann::json& os) {
			os["Left"].get_to(Left);
			os["Top"].get_to(Top);
			os["Right"].get_to(Right);
			os["Bottom"].get_to(Bottom);
		}
		...
		static Rectangle< T > FromJsonFile(nlohmann::json& os) {
			Rectangle < T > v;
			os["Left"].get_to(v.Left);
			os["Top"].get_to(v.Top);
			os["Right"].get_to(v.Right);
			os["Bottom"].get_to(v.Bottom);
			return v;
		}
	};
